...
interface CoBuyPaymentSuccessContentProps {
  shareToken: string;
}

function CoBuyPaymentSuccessContent({ shareToken }: CoBuyPaymentSuccessContentProps) {
  ...
  useEffect(() => {
    const confirmPayment = async () => {
      try {
        ...
        const json = await api.cobuy.confirmPayment(requestData);
        if (!json?.success) {
        ...
        setStatus('error');
        router.replace(`${routes.cobuyShareFail(shareToken)}?code=${encodeURIComponent(code)}&message=${encodeURIComponent(message)}`);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-sm p-8 text-center">
        ...
        <button
          type="button"
          onClick={() => router.push(routes.cobuyShare(shareToken))}
...
interface CoBuyPaymentSuccessPageClientProps {
  shareToken: string;
}

export default function CoBuyPaymentSuccessPageClient({ shareToken }: CoBuyPaymentSuccessPageClientProps) {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
      ...
      <CoBuyPaymentSuccessContent shareToken={shareToken} />
    </Suspense>
  );
}
