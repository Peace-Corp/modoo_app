...
interface CoBuySessionDetailClientProps {
  sessionId: string;
}

export default function CoBuySessionDetailClient({ sessionId }: CoBuySessionDetailClientProps) {
  ...
  const copyShareLink = () => {
    if (!session) return;
    const shareUrl = `${window.location.origin}${routes.cobuyShare(session.share_token)}`;
  ...
  const handleCloseSession = async () => {
    ...
    if (updated) {
      setSession(updated);
      api.cobuy.notifySessionClosed({ sessionId: updated.id })
  ...
  const handleCreateOrders = () => {
    ...
    router.push(routes.cobuyCheckout(session.id));
  };
